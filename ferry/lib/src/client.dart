...
class Client {
      ...
      case FetchPolicy.CacheAndNetwork:
        {
          final responseStreamFromNetwork =
              _optimisticNetworkResponseStream(operationRequest).shareValue();
          return _cacheResponseStream(operationRequest)
              .doOnData(_runUpdateCacheHandlers)
  ...
  /// Fetches the operation from the network, mapping the result to a
  /// [OperationResponse].
  Stream<OperationResponse<TData, TVars>> _networkResponseStream<TData, TVars>(
    OperationRequest<TData, TVars> operationRequest,
  ) async* {
    try {
      await for (var response in link.request(operationRequest.execRequest)) {
